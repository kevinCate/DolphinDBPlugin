...
static inline long long toMillis(bool isNano, long long v){
...
namespace ddb {
    ...
    // ---------- 修复后的 ReplyModeGuard：OFF 只写不读；ON 写+读一次OK ----------
    struct ReplyModeGuard {
        redisContext* c;
        bool armed{false};

        explicit ReplyModeGuard(redisContext* ctx): c(ctx) {}

        void off() {
            // 只 append + flush；OFF 下不会有回复，不能 getReply
            if (redisAppendCommand(c, "CLIENT REPLY OFF") == REDIS_ERR) {
                throw RuntimeException(std::string("[Plugin::Redis] append CLIENT REPLY OFF failed: ") + c->errstr);
            }
            int done = 0;
            while (!done) {
                if (redisBufferWrite(c, &done) == REDIS_ERR) {
                    throw RuntimeException(std::string("[Plugin::Redis] write CLIENT REPLY OFF failed: ") + c->errstr);
                }
            }
            armed = true;
            std::fprintf(stderr, "[Plugin::Redis] CLIENT REPLY OFF ok\n"); std::fflush(stderr);
        }

        void on() {
            if (!armed) return;
            if (redisAppendCommand(c, "CLIENT REPLY ON") == REDIS_ERR) {
                throw RuntimeException(std::string("[Plugin::Redis] append CLIENT REPLY ON failed: ") + c->errstr);
            }
            int done = 0;
            while (!done) {
                if (redisBufferWrite(c, &done) == REDIS_ERR) {
                    throw RuntimeException(std::string("[Plugin::Redis] write CLIENT REPLY ON failed: ") + c->errstr);
                }
            }
            // ON 会返回 OK，这里读一次
            redisReply* r = nullptr;
            if (redisGetReply(c, (void**)&r) != REDIS_OK) {
                throw RuntimeException(std::string("[Plugin::Redis] CLIENT REPLY ON getReply failed: ") + c->errstr);
            }
            RedisReplyGuard g(r);
            std::fprintf(stderr, "[Plugin::Redis] CLIENT REPLY ON reply type=%d\n", r ? r->type : -1);
            std::fflush(stderr);
            armed = false;
        }
        ...
        ~ReplyModeGuard() {
            if (!armed) return;
            // 最佳努力恢复：发 ON 并 flush；不要在析构里阻塞读
            if (redisAppendCommand(c, "CLIENT REPLY ON") == REDIS_OK) {
                int done = 0;
                while (!done) {
                    if (redisBufferWrite(c, &done) == REDIS_ERR) break;
                }
            }
        }
    };
    ...
    ConstantSP RedisConnection::redisBatchPush(const vector<ConstantSP> &args) {
        ...
        while (start < len) {
            ...
            for (int i = 0; i < count; ++i) {
                redisReply *reply;
                if (redisGetReply(redisConnect_, reinterpret_cast<void**>(&reply)) != REDIS_OK) {
                    string connMsg = (redisConnect_->err) ? CONN_ERR : ".";
