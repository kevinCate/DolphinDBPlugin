...
std::string_view extract_tag(std::string_view k) noexcept {
    const auto l = k.find('{');
    if (l == std::string_view::npos) return {};
    const auto r = k.find('}', l + 1);
    if (r == std::string_view::npos || r == l + 1) return {};  // 无右括号或空标签 {}
    return k.substr(l + 1, r - (l + 1)); // 子串 -> 指向 k，自身不拥有
}

// 需要“拥有”的版本（极少数场景才用）
std::string extract_tag_owned(std::string_view k) {
    const auto v = extract_tag(k);
    return v.empty() ? std::string{} : std::string{v};
}

std::string_view hash_basis(std::string_view k) noexcept {
    auto t = extract_tag(k);
    return t.empty() ? k : t; // TODO: warning
}

// CRC16-XMODEM for cluster slot calc
...
std::vector<SingleSlotGroup> group_by_single_slot(const std::vector<std::string>& keys){
    std::vector<SingleSlotGroup> out;
    ...
    by_tag.reserve(keys.size());

    for (std::size_t i = 0; i < keys.size(); ++i) {
        const std::string_view sv{keys[i]};
        const auto tag = extract_tag(sv);
        if (tag.empty()) no_tag.push_back(i);
        else by_tag[tag].rows.push_back(i);
    }

    for (auto& kv : by_tag) {
        if (kv.second.rows.empty()) continue;
        SingleSlotGroup g;
        g.routeKey = kv.first;                // bind pipelines to the tag
        g.rows = std::move(kv.second.rows);
    ...
    for (auto i : no_tag) {
        int slot = key_slot(std::string_view{keys[i]});
    ...
    for (auto& kv : by_slot) {
        ...
        SingleSlotGroup g;
        g.rows = std::move(kv.second.rows);
        g.routeKey = keys[g.rows.front()];    // representative key for this slot
        out.emplace_back(std::move(g));
    }
    return out;
}
